...
source("/path/to/custom_magma.R")
source("/path/to/robust_nmf_programs.R")

human_datasets = c() # list of human samplee IDs

# Load custom data
setwd("/path/to/all/NMF/weights/files/")
Genes_nmf_w_basis <- list()
for (i in dir(getwd())) {
  if (!(substr(i,1,10) %in% human_datasets)) {
    a <- read.csv(i, sep = ",", header = TRUE)
    rownames(a) <- a[,"X"]
    a <- a[,-1]
    Genes_nmf_w_basis[[paste(substr(i,1,10),"_rank4_9_nruns10.RDS", sep = "")]] <- a
  }
}
## Input:
...
nmf_programs <- read.csv("../mouse_robust_programs.csv", header = T)
...
write.csv(MP_list, "/path/to/save/mouse_MPs.csv")

Cluster_list <- do.call(cbind, lapply(lapply(Cluster_list, unlist), `length<-`, max(lengths(Cluster_list))))

write.csv(Cluster_list, "/path/to/save/mouse_MP_list.csv")
